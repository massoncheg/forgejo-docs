...
```sh
$ curl -H "Content-Type: application/json" -d '{"name":"test","scopes":["write:package"]}' -u username:password https://forgejo.example.org/api/v1/users/<username>/tokens
...
```sh
$ curl --url https://yourusername:password@forgejo.example.org/api/v1/users/<username>/tokens
...
```sh
$ curl -H "X-Forgejo-OTP: 123456" --url https://yourusername:yourpassword@forgejo.example.org/api/v1/users/yourusername/tokens
```
